...
def test_list_handles_history_load_runtime_error(
    xpiano_home: Path,
    monkeypatch,
) -> None:
    setup_result = runner.invoke(
    ...
    assert list_result.exit_code == 0
    assert "twinkle" in list_result.stdout
    assert "history backend crashed" not in list_result.stdout
...
def test_report_command_surfaces_report_read_oserror(
    ...
    monkeypatch.setattr(
        "xpiano.cli.latest_valid_report_path",
        lambda **kwargs: Path("/tmp/unreadable_report.json"),
    )
    monkeypatch.setattr(
        "xpiano.cli.load_report",
...
def test_record_full_tier_surfaces_save_coaching_oserror(
    ...
    monkeypatch.setattr(
        "xpiano.cli.save_coaching",
        lambda coaching, song_id, data_dir=None: (_ for _ in ()).throw(OSError("permission denied")),
    )
...
def test_record_full_tier_surfaces_stream_runtime_error(
    sample_midi_path: Path,
    monkeypatch,
) -> None:
    result = runner.invoke(
    ...
    monkeypatch.setattr("xpiano.cli.create_provider", lambda cfg: object())
    monkeypatch.setattr(
        "xpiano.cli._stream_coaching_text",
        lambda **kwargs: (_ for _ in ()).throw(RuntimeError("loop closed")),
    )
    record_result = runner.invoke(
        app, ["record", "--song", "twinkle", "--segment", "default"])
    assert record_result.exit_code != 0
    assert record_result.exception is not None
    assert not isinstance(record_result.exception, RuntimeError)
...
def test_record_full_tier_recovers_batch_coaching_when_stream_invalid(
    sample_midi_path: Path,
    monkeypatch,
) -> None:
    result = runner.invoke(
    ...
    monkeypatch.setattr(
        "xpiano.cli.save_coaching",
        lambda coaching, song_id, data_dir=None: Path("/tmp/recovered_stream_coaching.json"),
    )
...
def test_record_rejects_out_of_range_meta_beats_per_measure_from_meta(
    sample_midi_path: Path,
    monkeypatch,
) -> None:
    monkeypatch.setattr(
...
def test_record_rejects_out_of_range_meta_bpm_from_meta(
    sample_midi_path: Path,
    monkeypatch,
) -> None:
    monkeypatch.setattr(
...
def test_record_rejects_unsupported_meta_beat_unit_from_meta(
    sample_midi_path: Path,
    monkeypatch,
) -> None:
    monkeypatch.setattr(
...
def test_coach_command_surfaces_save_coaching_oserror(
    xpiano_home: Path,
    monkeypatch,
) -> None:
    reports_dir = xpiano_home / "songs" / "twinkle" / "reports"
...
def test_compare_handles_non_finite_history_rows(monkeypatch) -> None:
    rows = [
        {
            "segment_id": "verse1",
            "filename": "prev.json",
...
def test_compare_with_playback_uses_filename_when_row_path_missing(
    xpiano_home: Path,
    monkeypatch,
) -> None:
    reports_dir = xpiano_home / "songs" / "twinkle" / "reports"
    ...
    prev_report.write_text("{}", encoding="utf-8")
    curr_report.write_text("{}", encoding="utf-8")
    prev_attempt = xpiano_home / "songs" / "twinkle" / "attempts" / "prev.mid"
    curr_attempt = xpiano_home / "songs" / "twinkle" / "attempts" / "curr.mid"
    prev_attempt.parent.mkdir(parents=True, exist_ok=True)
    prev_attempt.write_bytes(b"prev")
    curr_attempt.write_bytes(b"curr")
    rows = [
        {
    ...
    monkeypatch.setattr("xpiano.cli.time.sleep", lambda seconds: None)
    monkeypatch.setattr(
        "xpiano.cli.midi_io.play_midi",
        lambda **kwargs: PlayResult(status="played", duration_sec=1.0),
    )
    result = runner.invoke(
        app,
        ["compare", "--song", "twinkle", "--playback", "--delay-between", "0"],
...
def test_compare_with_playback_uses_data_dir_for_relative_row_path(
    xpiano_home: Path,
    monkeypatch,
) -> None:
    reports_dir = xpiano_home / "songs" / "twinkle" / "reports"
    ...
    rows = [
        {
            ...
            "matched": 4,
            "ref_notes": 10,
            "path": "songs/twinkle/reports/prev_report.json",
        },
        {
            "filename": "curr_report.json",
            "segment_id": "verse1",
            ...
            "matched": 7,
            "ref_notes": 10,
            "path": "songs/twinkle/reports/curr_report.json",
        },
    ]
    monkeypatch.setattr("xpiano.cli.build_history", lambda **kwargs: rows)
    monkeypatch.setattr(
...
def test_compare_with_playback_recovers_stale_absolute_attempt_paths(
    ...
    stale_prev = "/stale-root/attempts/prev.mid"
    stale_curr = "/stale-root/attempts/curr.mid"
    monkeypatch.setattr("xpiano.cli.build_history", lambda **kwargs: rows)
    monkeypatch.setattr(
        "xpiano.cli.load_report",
        lambda path: {
            "inputs": {
                "attempt_mid": stale_prev if "prev_report" in str(path) else stale_curr
            }
        },
    )
    monkeypatch.setattr("xpiano.cli.mido.MidiFile", lambda path: f"midi::{Path(path).name}")
...
def test_compare_with_playback_skips_when_resolved_report_missing(
    tmp_path: Path,
    monkeypatch,
) -> None:
    prev_report = tmp_path / "prev_report.json"
    ...
    monkeypatch.setattr("xpiano.cli.load_report", _load_report)
    result = runner.invoke(
        app,
        ["compare", "--song", "twinkle", "--playback"],
    )
    assert result.exit_code == 0
    assert "Playback skipped: resolved report file not found." in result.stdout


def test_compare_with_playback_skips_when_resolved_report_invalid(
    tmp_path: Path,
    monkeypatch,
) -> None:
    prev_report = tmp_path / "prev_report.json"
    ...
    assert result.exit_code == 0
    assert "Playback skipped: resolved report file is invalid." in result.stdout
